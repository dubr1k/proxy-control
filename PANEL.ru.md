# Панель управления Proxy Control

Панель управляет отдельными границами Telemt/MTProto, NaiveProxy/Caddy, Mieru/mita и fleet. Возможности и семантика accounting для разных протоколов не взаимозаменяемы.

Панель доступна только на loopback-адресе хоста: `http://127.0.0.1:8787`. Для удалённого доступа используйте SSH-туннель (`ssh -L 8787:127.0.0.1:8787 server`) либо собственный HTTPS reverse proxy. Не публикуйте порт Telemt `9091`: в Compose он намеренно не имеет `ports`.

## Первый запуск

Генератор установки создаёт `secrets/telemt-api-token` с режимом `0600`; значение не попадает в `.env`, state-файл или логи. Не используйте пароль из примеров или production-конфигурации. Создайте первого владельца, передав новый пароль через stdin:

```sh
read -rsp 'Новый пароль: ' PANEL_INITIAL_PASSWORD; echo
# Именно exec, а не run: entrypoint образа игнорирует команду контейнера и
# всегда запускает uvicorn, поэтому `run` просто поднял бы ещё один сервер.
docker compose up -d panel
printf '%s\n' "$PANEL_INITIAL_PASSWORD" | docker compose exec -T panel \
  python -m panel.cli create-admin --username owner --role owner --password-stdin
unset PANEL_INITIAL_PASSWORD
docker compose up -d
```

Пароль должен содержать минимум 12 символов и хранится как Argon2id. В SQLite находятся только администраторы, opaque-сессии в виде SHA-256 digest, login throttling и аудит. Секреты прокси никогда не сохраняются панелью: их создаёт Telemt, а панель держит reveal только в памяти не более 120 секунд и отдаёт один раз.

## Настройки и роли

- `PANEL_ALLOWED_HOSTS` — допустимые Host через запятую; при reverse proxy добавьте публичное имя.
- `PANEL_COOKIE_SECURE=true` — оставляйте `true` при HTTPS; для прямой локальной HTTP-проверки временно задайте `false`.
- `PANEL_DATABASE=/data/panel.sqlite3` — SQLite на volume `panel-data`.
- `TELEMT_API_TOKEN_FILE=/run/secrets/telemt-api-token` — передача внутреннего API-токена.
- `PANEL_FLEET_HEARTBEAT_SECONDS=15` — как часто центральная панель опрашивает каждую связанную панель и доставляет ей недоставленные изменения (минимум 1; см. [FLEET.ru.md](FLEET.ru.md)).
- `PANEL_VERSION_FILE=/app/VERSION` — версия выпуска, которую эта панель сообщает центру; `compose.yaml` монтирует туда `./VERSION`, отсутствующий файл читается как `dev`.

`owner` управляет администраторами и пользователями; `admin` — пользователями и аудитом; `viewer` — только просмотр. Последнего активного owner нельзя удалить или понизить. Отключение администратора удаляет его сессии. Все мутации требуют CSRF и записываются в аудит без паролей, токенов, ссылок и proxy secrets.

`GET /api/audit` сохраняет обратно совместимый read-only ответ `items` и поддерживает `limit` (1–200), `before_id`, а также equality-фильтры `actor`, `action` и `target` (`actor` регистронезависим). Если есть следующая страница, `next_cursor` нужно передать как новый `before_id`.

С v0.3 панель может управлять другими панелями: центр добавляет панель по URL и API-ключу `node-sync`, импортирует её пользователей, выдаёт, ротирует и отзывает на ней доступы; связанная панель сохраняет своего владельца и свой UI. Вся модель — связанные панели, поколения, heartbeat, legacy mTLS-транспорт v1 — описана в [FLEET.ru.md](FLEET.ru.md).

## API-ключи и Bearer-аутентификация

Каждый маршрут `/api/*` принимает `Authorization: Bearer <ключ>` вместо cookie сессии.
Ключи создаёт владелец на экране «Администраторы», секция **«API-ключи»** («Создать
ключ»), либо через `POST /api/keys {name, scope, expires_at}`:

- **Scope** определяет роль запроса. `admin` действует как владелец (владелец без сессии —
  берегите его как пароль владельца), `monitor` — как наблюдатель (только чтение), а
  `node-sync` достаёт **только** до `/api/fleet/v2/*` — любой другой путь отвечает 403.
  Центральной панели нужен ключ `node-sync` от каждой панели, которой она управляет, и
  ничего больше.
- **Плейнтекст показывается один раз** в виде `pc_<prefix>_<secret>`; в базе хранятся
  8 hex-символов префикса для поиска и SHA-256 всего ключа. Потерянный ключ заменяется, а
  не восстанавливается. `GET /api/keys` отдаёт имя, scope, срок, статус, последнее
  использование и поисковый `prefix` — никогда сам ключ.
- **Отзыв немедленный**: выключенный (`POST /api/keys/{id}/enabled {enabled: false}`),
  удалённый (`DELETE /api/keys/{id}`) или просроченный ключ получает 401 на следующем
  запросе.
- **Rate limit**: 120 запросов в минуту на ключ (429), счёт ведётся в процессе панели.
- **Без CSRF для Bearer**: запрос с ключом не несёт cookie, поэтому проверка
  `X-CSRF-Token` к нему не применяется; проверка роли — применяется.
- **Аудит**: ключ действует как `key:<имя>` в `actor_username`; тела запросов не
  записываются. `last_used_at` обновляется не чаще раза в минуту.

Управление ключами (`/api/keys*`) требует роли владельца.

Для owner/admin раздел «Подключения» позволяет создавать, блокировать, разблокировать, ротировать и удалять отдельные доступы. Действующую Telegram-ссылку и QR-код можно открыть повторно явной кнопкой «QR и ссылка»; каждое такое раскрытие записывается в аудит, но сама ссылка и secret в журнал не попадают. Списки пользователей никогда не содержат ссылок или секретов.

## Трафик, квоты и лимиты Telemt 3.4.25

Панель намеренно показывает два разных счётчика:

- `runtime_total_octets` берётся из `GET /v1/users` (`total_octets`) и суммируется на дашборде как трафик текущего runtime-поколения Telemt. Обычно отсчёт начинается при запуске процесса, но in-runtime reload в 3.4.25 создаёт новое поколение статистики и тоже начинает этот счётчик заново. Это диагностическая runtime-метрика, а не расход квоты; ручной сброс квоты её не обнуляет.
- `quota_used_bytes` берётся из `GET /v1/stats/users/quota` (`used_bytes`) и показывается рядом с `data_quota_bytes`. Это сбрасываемый счётчик, по которому Telemt применяет квоту. `quota_last_reset_epoch_secs` — время последнего ручного сброса, либо `0`, если сброса ещё не было.

Кнопка «Сбросить квоту» вызывает `POST /v1/users/{username}/reset-quota`: Telemt обнуляет quota usage и сразу сохраняет quota-state, не меняя настроенный размер квоты и runtime `total_octets`. В Telemt 3.4.25 нет периодической записи quota-state: он сохраняется при явном сбросе и штатной остановке. После аварийного завершения возможна потеря расхода, накопленного после последнего сохранения. Автоматический дневной или ежемесячный сброс этой панелью не заявляется и не эмулируется; используйте только проверенную внешнюю автоматизацию, если нужен календарный период.

Форма лимитов меняет только документированные поля Telemt: quota bytes, up/down bits per second, TCP connections, unique IPs и RFC3339 expiration. Пустое поле отправляет `null` и снимает соответствующий override. Ответы панели формируются по явным allowlist-полям: Telemt `links`, `secret`, ad tags, списки IP и любые неизвестные или будущие вложенные поля в list/update/reset responses наружу не проходят.

## Дополнительное управление NaiveProxy

Интеграция с host Caddy подключается отдельным Docker override, поэтому обычная MTProxy-установка без NaiveProxy не ломается:

```sh
COMPOSE_FILE=compose.yaml:compose.naive.yaml docker compose up -d --build
```

На production-сервере эти значения удобно сохранить в локальном `.env` (он исключён из Git):

```dotenv
COMPOSE_FILE=compose.yaml:compose.naive.yaml
NAIVE_PUBLIC_HOST=proxy.example.com
NAIVE_DATA_DIR=/var/lib/naive-manager
```

`naive-manager` работает отдельным непривилегированным контейнером, использует host network только для loopback Caddy Admin API/TLS-probe и не получает Docker socket. Запись разрешена только в `NAIVE_DATA_DIR` и private runtime socket volume. `/var/log/naive-proxy` подключён read-only как `/logs`; SQLite и WAL учёта лежат под `/data` с mode `0600`. Учитываются только завершённые успешные CONNECT-записи управляемых пользователей: `bytes_read` — client→proxy, `size` — proxy→client. Это payload-байты закрытых туннелей, а не TLS/IP traffic.

Для NaiveProxy в Control Panel поле «Квота, MiB» задаёт per-user `quota_bytes`. `null` означает отключённую квоту и безлимитный режим; ноль и отрицательные значения не принимаются. Отдельный поток manager'а собирает завершённые CONNECT-записи по интервалу (по умолчанию 60 с, `NAIVE_QUOTA_INTERVAL_SECONDS`) даже при закрытой панели и при `total_bytes >= quota_bytes` транзакционно отключает пользователя, удаляя его credentials из управляемого Caddy-блока. Enforcement никогда не выполняется на read-путях: `GET /v1/health` и `GET /v1/traffic` только сообщают состояние и не переписывают управляемый конфиг. Это admission enforcement, а не побайтовый hard cap: уже открытый туннель может завершиться и дать overshoot, а пользователь может передавать данные до следующего прохода enforcement. Поэтому квота не должна использоваться как точный биллинговый лимит.

Сброс Naive-трафика обнуляет только локальный accounting baseline и не включает автоматически пользователя, отключённого по квоте. После сброса нужно явно нажать «Включить». Удаление квоты (`null`) или её увеличение выше зафиксированного расхода тоже не включают доступ: manager лишь фиксирует, что квота больше не удерживает пользователя (`disabled_reason` становится `manual`), а включение остаётся отдельной операцией. Попытка включить пользователя, у которого расход всё ещё превышает квоту, отклоняется с `409` и кодом причины `quota_exhausted` — панель показывает это как понятное действие, а не как отказ manager'а. Пароль, username и ссылка доступа в списке и ответах quota API не возвращаются.

Manager разворачивается раньше панели: старый manager молча игнорирует `quota_bytes` при создании и не имеет quota-эндпоинта, поэтому более новая панель сообщила бы об ошибке, с которой оператор ничего не сделает.

Production-контракт учёта хранит десять ротаций Caddy по 10 MiB и активный файл (заявленный footprint 110 MiB), а на точную проверку уже обработанных префиксов одного запроса выделяет не более 128 MiB. Проверка выполняется синхронно и использует общий request-wide budget для всех сохранённых файлов; дополнительные 18 MiB — ограниченный запас для активного файла, пересёкшего границу ротации. При старте budget меньше заявленного footprint отклоняется. Изменение префикса или превышение ограниченного budget переводит учёт в fail-closed unhealthy state вместо выдачи счётчиков с возможным повтором или пропуском.

- `Caddyfile` — источник истины Caddy с управляемым блоком `NAIVE-MANAGER USERS`;
- `users.json` — root/manager-only состояние, включая отключённые ключи;
- `backups/` — парные снимки Caddyfile и `users.json` перед каждой мутацией (хранятся последние 20 транзакций);
- `transaction.json` — fsync-журнал незавершённой транзакции; после сбоя manager при старте либо восстанавливает оба предыдущих файла, либо повторно загружает уже полностью записанное новое поколение;
- `manager-token` — копия внутреннего токена с режимом `0400` для UID manager-контейнера.

Перед первым запуском скопируйте действующий Caddyfile в `${NAIVE_DATA_DIR}/Caddyfile`, создайте `secrets/naive-manager-token` с режимом `0600` и передайте такую же копию как `${NAIVE_DATA_DIR}/manager-token`. Restrictive `umask 077` не должен делать source/build context нечитаемым для UID `10002`. Bootstrap использует Caddy Admin `/adapt`, поэтому сначала запускается host Caddy из legacy-credential generation, затем выполняется import:

```sh
NAIVE_DATA_DIR=${NAIVE_DATA_DIR:-/var/lib/naive-manager}
test ! -L "${NAIVE_DATA_DIR}" || { echo "NAIVE_DATA_DIR must not be a symlink" >&2; exit 1; }
# Завершаем работу при коллизии фиксированных production ID.
uid_name=$(getent passwd 10003 | cut -d: -f1 || true)
gid_name=$(getent group 10004 | cut -d: -f1 || true)
test -z "${uid_name}" -o "${uid_name}" = naive-caddy || { echo "UID 10003 collision: ${uid_name}" >&2; exit 1; }
test -z "${gid_name}" -o "${gid_name}" = naive-accounting || { echo "GID 10004 collision: ${gid_name}" >&2; exit 1; }
getent group naive-accounting >/dev/null || groupadd --system --gid 10004 naive-accounting
id naive-caddy >/dev/null 2>&1 || useradd --system --uid 10003 --gid naive-accounting --home /nonexistent --shell /usr/sbin/nologin naive-caddy
test "$(id -u naive-caddy)" = 10003 || { echo "naive-caddy must use UID 10003" >&2; exit 1; }
test "$(id -g naive-caddy)" = 10004 || { echo "naive-caddy must use GID 10004" >&2; exit 1; }
# UID 10002/GID 101 остаются identity manager; GID 10004 даёт только чтение логов.
install -d -o 10002 -g 101 -m 0700 "${NAIVE_DATA_DIR}"
install -d -o 10003 -g 10004 -m 0750 /var/log/naive-proxy
for file in Caddyfile manager-token; do
  test -f "${NAIVE_DATA_DIR}/${file}" && test ! -L "${NAIVE_DATA_DIR}/${file}" || exit 1
done
chown -h 10002:101 "${NAIVE_DATA_DIR}/Caddyfile" "${NAIVE_DATA_DIR}/manager-token"
chmod 0640 "${NAIVE_DATA_DIR}/Caddyfile"
chmod 0400 "${NAIVE_DATA_DIR}/manager-token"
# Служба запускает /usr/local/bin/caddy и до этого проверяет его сборку,
# поэтому зафиксированный бинарник должен быть установлен заранее:
#   docker buildx build --file docker/Dockerfile.caddy-naive \
#     --output type=local,dest=/tmp/pc-caddy .
#   install -o root -g root -m 0755 /tmp/pc-caddy/caddy /usr/local/bin/caddy
test -x /usr/local/bin/caddy
install -o root -g root -m 0755 scripts/check-naive-caddy-build.sh /usr/local/libexec/check-naive-caddy-build
install -o root -g root -m 0755 scripts/caddy-naive-adapt /usr/local/libexec/caddy-naive-adapt
install -o root -g root -m 0644 deploy/caddy-naive.service /etc/systemd/system/caddy-naive.service
systemctl daemon-reload
systemctl enable --now caddy-naive
test "$(ss -H -lnt 'sport = :2019' | awk '{print $4}')" = 127.0.0.1:2019
test "$(ss -H -lnt 'sport = :4443' | awk '{print $4}')" = 127.0.0.1:4443
docker compose -f compose.yaml -f compose.naive.yaml run --rm --build naive-manager --bootstrap-only
systemctl reload caddy-naive
# Выполните один authenticated public CONNECT, закройте туннель и потребуйте:
test -f /var/log/naive-proxy/access.json
test "$(stat -c %a /var/log/naive-proxy/access.json)" = 640
docker compose -f compose.yaml -f compose.naive.yaml up -d --build --wait
```

Текущие `caddy-naive-adapt` и `naive_manager` отключают automatic HTTPS redirects в private generation. Без этого непривилегированный Caddy пытается занять `127.0.0.1:80`. Initial bootstrap записывает managed credentials/accounting, но не активирует их: обязательный `systemctl reload caddy-naive` должен предшествовать запуску long-running manager, а первый завершённый CONNECT — проверке accounting health.

Host Caddy и manager-контейнер имеют разные identity: Caddy использует UID `10003`, manager остаётся `10002:101` и получает только supplementary accounting GID `10004`. `/var/log/naive-proxy` имеет owner `10003:10004` и mode `0750`; Caddy создаёт логи mode `0640`, поэтому manager может читать, но не создавать, обрезать, переименовывать или дописывать их. Manager data остаётся `10002:101`, mode `0700`. При старте и systemd reload привилегированный `install` staging-копирует manager-owned source в `/run/caddy-naive/Caddyfile` (`10003:10004`, `0400`) внутри runtime directory mode `0700`; Caddy runtime не может пройти в `/var/lib/naive-manager`. Manager mutations не используют staged-файл: свежий source адаптируется и валидируется через loopback Admin API, а тот же JSON отправляется в `/load`, поэтому stale-stage window отсутствует и transaction rollback сохраняется. До cutover выполните `systemd-analyze verify`, проверьте `systemctl show caddy-naive -p User -p Group`, невозможность manager дописать лог и невозможность Caddy прочитать manager state.

Для миграции сначала остановите manager mutations и Caddy, затем сохраните unit, Caddy binary, Caddyfile, manager data и log directory. До смены ownership выполните UID/GID collision preflight, создайте разделённые identity и permissions, установите pinned binary/checker, выполните checker и точный `caddy adapt --validate`, bootstrap manager, переключите unit, затем запустите Caddy и Compose override. Повторный bootstrap идемпотентен; сбой на фазах prepared, files-replaced или reload-pending восстанавливает парное старое поколение config/state перед повтором. Старый unit и backup не удаляйте до успешных health, cover HTTPS, authenticated CONNECT, traffic collection и регрессии соседних SNI. Дальнейшие изменения идут через paired backup → adapt/validate → fsync journal → atomic replace → `/load` → HTTPS probe; неподтверждённый rollback оставляет manager unhealthy и сохраняет journal.

Rollback выполняется на host: остановите `naive-manager` и Caddy; одной генерацией восстановите Caddyfile/unit/binary, snapshot manager data, ownership/modes логов и прежние service identity; провалидируйте восстановленным build; перезапустите Caddy и повторите cover/authenticated/SNI probes. Не копируйте один `traffic.sqlite3` без `-wal`/`-shm` при работающем manager. Reset меняет только локальный baseline; viewer reset запрещён, audit сохраняет только action/username без credentials и authorization headers.

One-time dialog разделяет форматы по клиентам:

- **Native** скачивает или копирует штатный NaiveProxy `config.json`; документированного QR-импорта у native client нет, поэтому QR на этой вкладке отсутствует:

  ```json
  {"listen":"socks://127.0.0.1:1080","proxy":"https://USER:PASSWORD@proxy.example.com"}
  ```
- **NekoBox** отдаёт ссылку `naive+https://USER:PASSWORD@HOST:443#NAME` и её QR. Это формат, который разбирает `parseNaive` в NekoBox for Android и его форках, поэтому импорт по QR там работает.
- **Karing** скачивает полный sing-box JSON profile и открывает `karing://install-config`. QR кодирует эту deep link с содержимым полного профиля, а не raw endpoint `https://USER:PASSWORD@HOST`.
- **Shadowrocket** показывает только проверяемые поля для ручного ввода (`HTTPS`, server, port, username, password). Proxy Control не придумывает неподтверждённую Shadowrocket URI или QR.

На вкладках без QR (Native, Shadowrocket) панель убирает QR-плашку целиком, а не показывает пустой белый прямоугольник: пустая плашка читается как сломанный код и провоцирует сканировать её не тем клиентом.

Текущий source Karing принимает `karing://install-config?url=...`, импортирует содержимое sing-box config и содержит Naive в protocol editor. См. [Karing URL scheme](https://karing.app/en/cooperation/scheme), [инструкцию импорта Karing](https://karing.app/en/quickstart) и [схему Naive outbound в sing-box](https://sing-box.sagernet.org/configuration/outbound/naive/). Все варианты содержат один credential и подчиняются one-time reveal и `Cache-Control: no-store`.

## Переключение писателя: `PANEL_VNEXT_WRITER`

С v0.2 у протокольных endpoint'ов два писателя, и переключаются они переменной
`PANEL_VNEXT_WRITER`:

- `legacy` (по умолчанию) — панель зовёт менеджеры напрямую, ровно как в v0.1.0;
- `domain` — те же запросы идут через клиентов и доступы, поэтому панель **владеет
  credential** и может отрендерить подписку.

Снаружи разница не видна: те же коды ответов, та же форма тела, те же действия в
журнале. Это проверяется тем, что весь API-набор тестов прогоняется в обоих режимах.

Порядок перехода — именно такой:

1. Импортируйте существующих пользователей на экране «Клиенты».
2. Примите доступы кнопкой «Принять» (для Mieru — с осознанной ротацией).
3. Поставьте `PANEL_VNEXT_WRITER=domain` и перезапустите панель.
4. Проверьте создание/включение/ротацию по одному пользователю каждого протокола.

Если на шаге 3 в базе есть пользователи, которых панель не знает, ничего страшного:
при первой же операции над таким пользователем панель запишет его как `imported`,
не создавая заново и не сливая с одноимённой учёткой другого протокола. Credential у
такой строки не появится — панель его не знает; примите доступ явно.

Элевированные Mieru-флаги (`allow_private_ip`, `allow_loopback_ip`) остаются на
прямом вызове менеджера и в `domain`: это осознанно ручной путь, и протаскивать его
через общий intent было бы подменой смысла.

## Подписки: `PANEL_SUBSCRIPTION_URL` и `/s/{token}`

Подписка клиента — одна ссылка `https://<домен подписки>/s/<token>`, которая рендерит
все доступы клиента из собственного хранилища панели — менеджеры при выдаче не вызываются.
Панель публикует её только при заданном `PANEL_SUBSCRIPTION_URL` (установщик выводит его из
`domains.subscription` в `install.toml`; хост становится `PANEL_SUBSCRIPTION_HOST` и добавляется в
`PANEL_ALLOWED_HOSTS`). Путь отвечает **только** на этом хосте: на домене панели `/s/` — 404, тот
же, что и для неизвестного или отозванного токена, поэтому подписка ничего не говорит о том, где
живёт панель. При пустой переменной endpoint выключен.

С v0.10 подписка выдаётся вместе с клиентом (окно «Новый клиент»), а её ссылка и QR доступны
снова в окне клиента по кнопке «Показать»: токен хранится зашифрованным под мастер-ключом
панели, показ доступен owner/admin и записывается в аудит как `subscription.reveal`. Без
мастер-ключа панель показывает ссылку один раз, как раньше. Там же, в окне клиента, — матрица
«узел × протокол»: галочка выдаёт или включает доступ, снятая галочка выключает его (аккаунт
остаётся), крестик удаляет; строки — этот сервер и связанные панели не на паузе, а узел на паузе
с доступами клиента показывается только для чтения. Доступ без сохранённого секрета подписку
не блокирует — он попадает в неё как `unsupported`.

С v0.12 блок подписки рисуется только тогда, когда подписке есть что отдавать: MTProxy в ней
не участвует — Telegram открывает ссылку `tg://proxy` и никогда не опрашивает URL подписки.
Клиенту с одним MTProxy вместо блока показывается строка о том, где взять ссылки, а уже выданная
подписка остаётся на экране, чтобы её можно было отозвать. Сами ссылки — в блоке «Ссылки MTProxy»
того же окна: `POST /api/clients/{id}/links` (owner/admin, необязательный `protocol`) собирает их
из хранилища панели, по одной на каждый узел, и отдаёт разовым reveal'ом вместе с QR.

Формат выбирается через `?format=` или `Accept`: `raw` (простой текст, по ссылке на строку,
без base64), `singbox` (`application/json`, outbound'ы naive + mieru для Karing; `&client=singbox`
урезает до одного naive, потому что официальный sing-box ≥ 1.13 не загружает конфиг с неизвестным
типом outbound), `clash` (`text/yaml`, mieru-прокси для mihomo), `manifest`
(`application/vnd.proxy-control.subscription+json;version=1`) и `html` (страница со ссылками,
QR и матрицей совместимости). То, что клиент не умеет разбирать, попадает в `unsupported` с
причиной, а не выдаётся нерабочей ссылкой; доступ без сохранённого credential —
`unsupported: no stored credential`, пока его не примут.

Ответы несут `ETag` (от эффективного набора доступов: он меняется, когда доступ добавлен,
отключён, ротирован или истёк, а не когда страницу прочитали), `Cache-Control: private, no-cache`,
`Profile-Update-Interval: 12` и `X-Robots-Tag: noindex`; `If-None-Match` получает 304. Лимит — 60
запросов в минуту с одного адреса по всем токенам сразу. Токен хранится хэшем, не логируется
(uvicorn без access log, у Nginx-блока `access_log off`) и не попадает в журнал аудита.

## Экран «Клиенты»

Клиент — это человек или устройство, а доступ (grant) — его учётная запись в одном
протоколе на одном узле. Карточка показывает имя клиента, состояние
(активен / приостановлен / в архиве) и чипы доступов «протокол · учётная запись ·
состояние».

Кнопка «Импорт существующих» открывает список того, что менеджеры уже используют.
Панель при этом **только читает**: пароли, квоты и включённость пользователей не
меняются, ни один `create`, `rotate`, `enable` или `delete` не вызывается. Поэтому
импорт безопасен на работающем сервере и его можно повторять — уже импортированные
доступы помечаются и пропускаются.

Одинаковое имя пользователя в разных протоколах — **только подсказка**. Панель
помечает такие строки, но по умолчанию каждая пара «протокол + учётная запись»
становится отдельным клиентом: три `alice` в трёх менеджерах не доказывают, что это
один человек, а молчаливое объединение выдало бы одному абоненту чужой доступ.
Объединить можно вручную, выбрав существующего клиента в колонке «Куда».

Импортированный доступ не содержит секрета: панель не знает пароль, который менеджер
выдал раньше. Такой доступ помечается «без секрета»; с v0.10 он не мешает выдать подписку —
попадает в неё как `unsupported`, пока его не примут кнопкой «Принять».

Принятие работает по-разному, и разница не косметическая:

- **MTProxy и NaiveProxy** отдают текущий секрет по запросу, поэтому панель просто
  читает его и кладёт в шифрованное хранилище. Ссылка, которая уже у клиента,
  продолжает работать.
- **Mieru** хранит только хеш пароля, поэтому прочитать его нельзя ни панели, ни
  самому mita. Единственный честный способ — выпустить новый пароль, и панель
  спрашивает подтверждение прямо: **старая `mierus://`-ссылка перестанет работать**,
  клиенту придётся выдать новую. Без подтверждения панель отказывает (409) и менеджер
  не трогает.

Ни в ответе API, ни в журнале действий пароль не появляется: аудит записывает только
протокол и факт `rotated: true|false`.

### Выдача доступов

Кнопка «Выдать доступ» на карточке клиента создаёт учётные записи сразу в нескольких
протоколах **одной операцией**. Три менеджера не умеют общую транзакцию, поэтому
«всё или ничего» здесь недостижимо — вместо этого операция ведёт журнал, и любой её
исход — ровно один из трёх:

- **succeeded** — все доступы созданы, секреты лежат в шифрованном хранилище;
- **compensated** — что-то отказало, и всё созданное **этой операцией** удалено. То,
  что существовало на сервере до неё, не трогается никогда;
- **manual_intervention_required** — рантайм изменился, но откатить безопасно нельзя.
  Панель называет id операции; продолжить можно командой
  `python -m panel.cli operations-resume <id>`.

Журнал переживает перезапуск: повторный `run` продолжает с последней отметки, а не
начинает заново, поэтому второй учётной записи с тем же именем не появится.

После успеха панель один раз показывает все ссылки операции. Повторно тот же токен
не открывается — это одноразовое раскрытие.

Узел v1 с неудалёнными доступами нельзя отключить: иначе абоненты потеряли бы связь
молча. Сначала переведите доступы в `deleted`, потом отключайте узел. Отключение
связанной панели лишь ставит связь на паузу (см. ниже).

Выбор узла в «Выдать доступ» включает связанные панели рядом с этим сервером. Доступ
на связанной панели декларативен: операция ждёт как `pending_remote`, а доступ
показывается как **«ожидает узел»**, пока та панель не сообщит, что учётная запись
применена; включение, выключение, ротация и удаление работают так же — через новое
поколение, а не через вызов менеджера ([FLEET.ru.md](FLEET.ru.md)).

## Экран «Узлы»

Экран перечисляет три вида узлов: этот сервер, **связанные панели** (Fleet v2) и legacy
**узлы v1** (mTLS-агент). Зарезервированный узел «Этот сервер» есть в каждой установке —
его создаёт миграция базы, а не оператор, — и он всегда стоит первым.

**Этот сервер.** Вместо сертификатов карточка показывает здоровье трёх менеджеров —
Telemt, NaiveProxy, Mieru: «работает», «недоступен» или «выключен» для протокола, который
не включён в этой установке, — и то, что нужно центру, чтобы подключить эту панель: её
GUID и URL для ввода там, с подсказкой создать ключ `node-sync`. Как только центр прислал
поколение, карточка показывает «управляется центром <GUID>» и, для владельца, кнопку
**«Отвязать»**: панель забывает мастера, учётные записи, которыми владел центр, снова
становятся локальными, в runtime ничего не меняется. Переименовать узел можно, отключить
или отозвать сертификаты — нет; `node_id` `local` зарезервирован.

**Связанные панели.** Кнопка **«+ Панель»** в шапке открывает диалог «Добавить панель»
(имя, `https://` URL, API-ключ, проверка TLS `verify` или `pin` с «Получить отпечаток»,
«Разрешить приватный адрес», **«Проверить»**, список для импорта). Карточка связанной
панели показывает online/offline с latency и последним heartbeat, версию панели узла,
здоровье демонов по протоколам, пользователей (центр против локальных), best-effort
трафик, поколение `desired` против `applied` и последнюю ошибку — в трёх вкладках:
**Обзор**, **Пользователи** (учётные записи узла с кнопкой **«Импортировать выбранных»**)
и **Обновления** (версии компонентов от version-agent узла с кнопкой обновления). Действия
владельца: **Пауза / Возобновить** (heartbeat и доставка пропускают связь на паузе;
«отключить» такой узел означает то же), **Проверить** (heartbeat и доставка сейчас),
**Изменить** (имя, URL, ключ, TLS) и **Удалить** — отказ, пока на панели остаются
выданные доступы, включая ещё не подтверждённые узлом удаления. API-ключ больше никогда не показывается (`has_api_key`).

**Узлы v1.** «Зарегистрировать узел v1 (mTLS-агент)» в тулбаре создаёт узел и тут же
показывает чек-лист enrollment; панель не умеет выпустить сертификат за вас — приватный
ключ генерируется на самом узле и никуда не копируется. Карточка показывает identity
(имя, `node_id`), состояние enrollment, транспорт (когда узел выходил на связь) и
состояние демонов, перечисляет сертификаты с датой окончания (истекающие в ближайшие две
недели подсвечены). **Переименовать** меняет только отображаемое имя — `node_id`
неизменен, к нему привязаны сертификаты и доступы. **Отключить / Включить**: отключённый
узел не проходит аутентификацию на транспорте; панель отказывает, пока у узла есть
незавершённые команды или неудалённые доступы. **Отозвать все сертификаты** требует
повторить `node_id`. Сырые typed-команды Telemt v1 живут в разделе «Advanced: транспорт
v1» карточки.

Подробности обоих транспортов: [FLEET.ru.md](FLEET.ru.md).

## Экран «Маршрутизация» (v0.4)

**«Маршрутизация»** задаёт для каждого узла и сервиса, куда уходит трафик клиентов:
выбор узла (этот сервер и связанные панели), вкладки NaiveProxy / Mieru / MTProxy
(последняя — «вне области маршрутизации»), и для каждой политики — умолчания («По
умолчанию: Напрямую | Через WARP», «При недоступности WARP: отказать | напрямую») и
упорядоченные правила — включено, домены, CIDR, порты, действие (Блокировать / Напрямую /
Через WARP), заметка — с ↑/↓ и перетаскиванием, «Добавить правило», «Сбросить» (напрямую,
без правил) и «Сохранить». Предпросмотр рядом с редактором обновляется на каждое изменение
(debounce 400 мс): статус, причины с привязкой к правилу, предупреждения (WARP не отвечает,
рукописный upstream будет перенят), diff относительно узла, нужен ли перезапуск и цель
отката. **«Применить»** активна только для сохранённой поддерживаемой политики;
**«Откатить»** — когда что-то уже применялось; **«История»** показывает `routing_applies`;
**«Удалить политику»** — только после «Сбросить» + «Применить». Бейдж карточки — «применено
(rev N)», «есть неприменённые изменения», «применяется…» или «ошибка: …»; для связанной
панели старше v0.4 — «узел нужно обновить до v0.4», а на узле, которым управляет центр,
экран говорит об этом и оставляет право записи центру. Карточка узла на «Узлах» получает
строку «Маршрутизация: naive → WARP, 2 блокир.; mieru → напрямую». Читать и делать
предпросмотр может любая роль; применяет owner. Что умеет каждый backend, API
(`/api/routing/*`) и события аудита: [docs/ROUTING.ru.md](docs/ROUTING.ru.md).

С v0.5 карточка показывает и **Xray-router** узла ([docs/XRAY_ROUTER.ru.md](docs/XRAY_ROUTER.ru.md)): бейдж
backend — Caddy, mita или Xray-router, а строка роутера говорит «Xray-router: не установлен / сервис не
подключён / сервис подключён» с версией Xray и кнопками только для owner **«Подключить к
Xray-router»** / **«Отключить от Xray-router»** (с подтверждением: сессии сервиса прервутся). У правила
появляются поля geosite, geoip и порты; на нативном backend'е предпросмотр отвечает «не применимо» с
упоминанием роутера. Связанная панель старше v0.5 для политики роутера показывает «узел нужно
обновить до v0.5 и установить Xray-router».

## Мастер-ключ и ротация

С v0.2 панель может сохранить credential клиента, чтобы подписка рендерилась
и завтра. Такие значения шифруются AES-256-GCM под ключом из
`secrets/panel-master-key`, который entrypoint кладёт в контейнер только на
чтение как `/run/panel/master-key`. Шифрование привязано к identity строки:
ciphertext, скопированный в другую строку или выданный другому узлу, не
расшифруется.

От чего защищает: кража файла БД или бэкапа. От чего нет: компрометация
самого процесса панели — он держит ключ, пока работает.

- Установщик создаёт ключ при установке и обновлении и никогда не пересоздаёт.
- Панель без сохранённых секретов стартует и без ключа; как только
  зашифрованные строки есть, а ключа нет, она отказывается стартовать, а не
  отдаёт пустые подписки.
- Копия ключа хранится отдельно от БД ([backup и
  restore](docs/BACKUP_RESTORE.ru.md)).

```sh
# проверить, что ключ и БД из одного поколения (печатает счётчики, не значения)
docker compose exec panel python -m panel.cli master-key-verify --path /run/panel/master-key

# ротация: новый активный ключ, перешифровка батчами, проверка, сужение keyring
docker compose exec panel python -m panel.cli master-key-rotate --path /run/panel/master-key
```

Ротация идёт overlap-first: файл держит оба ключа, пока строки
перешифровываются, поэтому прерванная ротация оставляет всё читаемым. После
завершения снимите свежую копию ключа.

## Резервное копирование

Резервируйте volumes `panel-data` и `telemt-config`, каталог `${NAIVE_DATA_DIR}` при включённой Naive-интеграции, а также secret-файлы отдельно с режимом `0600`. `users.conf` импортируется только при первом создании `telemt-config/config.toml`; далее источник истины — конфигурация Telemt, которую его API меняет атомарно. Удаление `telemt-config` приводит к повторному импорту исходного `users.conf`.

```sh
curl -fsS -H 'Host: panel.example.com' http://127.0.0.1:8787/healthz
docker compose ps
docker compose logs panel mtproxy   # вывод не должен содержать secrets
```
