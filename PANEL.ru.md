...
## Переключение писателя: `PANEL_VNEXT_WRITER`

С v0.2 у протокольных endpoint'ов два писателя, и переключаются они переменной
`PANEL_VNEXT_WRITER`:

- `legacy` (по умолчанию) — панель зовёт менеджеры напрямую, ровно как в v0.1.0;
- `domain` — те же запросы идут через клиентов и доступы, поэтому панель **владеет
  credential** и может отрендерить подписку.

Снаружи разница не видна: те же коды ответов, та же форма тела, те же действия в
журнале. Это проверяется тем, что весь API-набор тестов прогоняется в обоих режимах.

Порядок перехода — именно такой:

1. Импортируйте существующих пользователей на экране «Клиенты».
2. Примите доступы кнопкой «Принять» (для Mieru — с осознанной ротацией).
3. Поставьте `PANEL_VNEXT_WRITER=domain` и перезапустите панель.
4. Проверьте создание/включение/ротацию по одному пользователю каждого протокола.

Если на шаге 3 в базе есть пользователи, которых панель не знает, ничего страшного:
при первой же операции над таким пользователем панель запишет его как `imported`,
не создавая заново и не сливая с одноимённой учёткой другого протокола. Credential у
такой строки не появится — панель его не знает; примите доступ явно.

Элевированные Mieru-флаги (`allow_private_ip`, `allow_loopback_ip`) остаются на
прямом вызове менеджера и в `domain`: это осознанно ручной путь, и протаскивать его
через общий intent было бы подменой смысла.

## Подписки: `PANEL_SUBSCRIPTION_URL` и `/s/{token}`

Подписка клиента — одна ссылка `https://<домен подписки>/s/<token>`, которая рендерит
все доступы клиента из собственного хранилища панели — менеджеры при выдаче не вызываются.
Панель публикует её только при заданном `PANEL_SUBSCRIPTION_URL` (установщик выводит его из
`domains.subscription` в `install.toml`; хост становится `PANEL_SUBSCRIPTION_HOST` и добавляется в
`PANEL_ALLOWED_HOSTS`). Путь отвечает **только** на этом хосте: на домене панели `/s/` — 404, тот
же, что и для неизвестного или отозванного токена, поэтому подписка ничего не говорит о том, где
живёт панель. При пустой переменной endpoint выключен.
...
QR и матрицей совместимости). То, что клиент не умеет разбирать, попадает в `unsupported` с
причиной, а не выдаётся нерабочей ссылкой; доступ без сохранённого credential —
`unsupported: no stored credential`, пока его не примут.

Ответы несут `ETag` (от эффективного набора доступов: он меняется, когда доступ добавлен,
отключён, ротирован или истёк, а не когда страницу прочитали), `Cache-Control: private, no-cache`,
`Profile-Update-Interval: 12` и `X-Robots-Tag: noindex`; `If-None-Match` получает 304. Лимит — 60
запросов в минуту с одного адреса по всем токенам сразу. Токен хранится хэшем, не логируется
(uvicorn без access log, у Nginx-блока `access_log off`) и не попадает в журнал аудита.

## Экран «Клиенты»

Клиент — это человек или устройство, а доступ (grant) — его учётная запись в одном
протоколе на одном узле. Карточка показывает имя клиента, состояние
(активен / приостановлен / в архиве) и чипы доступов «протокол · учётная запись ·
состояние».

Кнопка «Импорт существующих» открывает список того, что менеджеры уже используют.
Панель при этом **только читает**: пароли, квоты и включённость пользователей не
меняются, ни один `create`, `rotate`, `enable` или `delete` не вызывается. Поэтому
импорт безопасен на работающем сервере и его можно повторять — уже импортированные
доступы помечаются и пропускаются.

Одинаковое имя пользователя в разных протоколах — **только подсказка**. Панель
помечает такие строки, но по умолчанию каждая пара «протокол + учётная запись»
становится отдельным клиентом: три `alice` в трёх менеджерах не доказывают, что это
один человек, а молчаливое объединение выдало бы одному абоненту чужой доступ.
Объединить можно вручную, выбрав существующего клиента в колонке «Куда».

Импортированный доступ не содержит секрета: панель не знает пароль, который менеджер
выдал раньше. Такой доступ помечается «без секрета» и не попадает в подписку, пока его
не примут кнопкой «Принять».

Принятие работает по-разному, и разница не косметическая:

- **MTProxy и NaiveProxy** отдают текущий секрет по запросу, поэтому панель просто
  читает его и кладёт в шифрованное хранилище. Ссылка, которая уже у клиента,
  продолжает работать.
- **Mieru** хранит только хеш пароля, поэтому прочитать его нельзя ни панели, ни
  самому mita. Единственный честный способ — выпустить новый пароль, и панель
  спрашивает подтверждение прямо: **старая `mierus://`-ссылка перестанет работать**,
  клиенту придётся выдать новую. Без подтверждения панель отказывает (409) и менеджер
  не трогает.

Ни в ответе API, ни в журнале действий пароль не появляется: аудит записывает только
протокол и факт `rotated: true|false`.

### Выдача доступов

Кнопка «Выдать доступ» на карточке клиента создаёт учётные записи сразу в нескольких
протоколах **одной операцией**. Три менеджера не умеют общую транзакцию, поэтому
«всё или ничего» здесь недостижимо — вместо этого операция ведёт журнал, и любой её
исход — ровно один из трёх:

- **succeeded** — все доступы созданы, секреты лежат в шифрованном хранилище;
- **compensated** — что-то отказало, и всё созданное **этой операцией** удалено. То,
  что существовало на сервере до неё, не трогается никогда;
- **manual_intervention_required** — рантайм изменился, но откатить безопасно нельзя.
  Панель называет id операции; продолжить можно командой
  `python -m panel.cli operations-resume <id>`.

Журнал переживает перезапуск: повторный `run` продолжает с последней отметки, а не
начинает заново, поэтому второй учётной записи с тем же именем не появится.

После успеха панель один раз показывает все ссылки операции. Повторно тот же токен
не открывается — это одноразовое раскрытие.

Узел с неудалёнными доступами нельзя отключить: иначе абоненты потеряли бы связь
молча. Сначала переведите доступы в `deleted`, потом отключайте узел.

## Экран «Узлы»

Карточка узла показывает четыре вещи: identity (имя, `node_id`, «Этот сервер» или
«Удалённый узел»), состояние enrollment, транспорт (когда узел выходил на связь) и
состояние демонов. Сертификаты перечислены с датой окончания; те, что истекают в
ближайшие две недели, подсвечены.

Действия владельца:

- **Зарегистрировать** (кнопка «Добавить») — создаёт узел и тут же показывает
  чек-лист enrollment. Панель не умеет выпустить сертификат за вас: приватный ключ
  генерируется на самом узле и никуда не копируется.
- **Переименовать** — меняет только отображаемое имя; `node_id` неизменен, к нему
  привязаны сертификаты и доступы.
- **Отключить / Включить** — отключённый узел не проходит аутентификацию на
  транспорте. Панель отказывает, пока у узла есть незавершённые команды — иначе
  они зависли бы в очереди без исполнителя.
- **Отозвать все сертификаты** — требует повторить `node_id` в диалоге; узел
  останется без транспорта, пока ему не выпустят новый сертификат.

Зарезервированный узел «Этот сервер» есть в каждой установке — его создаёт миграция
базы, а не оператор, и он всегда стоит первым в списке. Действий enrollment у него
нет вообще: локальные протоколы управляются напрямую, без очереди команд и без
сертификатов. Вместо списка сертификатов карточка показывает здоровье трёх
менеджеров — Telemt, NaiveProxy, Mieru: «работает», «недоступен» или «выключен»
для протокола, который не включён в этой установке. Переименовать его можно,
отключить или отозвать сертификаты — нет; `node_id` `local` зарезервирован, и
зарегистрировать под ним обычный узел панель откажется.

Сырые typed-команды Telemt v1 живут в разделе «Advanced: транспорт v1» внутри карточки:
это тот же интерфейс, что и раньше, просто не на первом экране. См. также
[FLEET.ru.md](FLEET.ru.md).

## Мастер-ключ и ротация

С v0.2 панель может сохранить credential клиента, чтобы подписка рендерилась
и завтра. Такие значения шифруются AES-256-GCM под ключом из
`secrets/panel-master-key`, который entrypoint кладёт в контейнер только на
чтение как `/run/panel/master-key`. Шифрование привязано к identity строки:
ciphertext, скопированный в другую строку или выданный другому узлу, не
расшифруется.

От чего защищает: кража файла БД или бэкапа. От чего нет: компрометация
самого процесса панели — он держит ключ, пока работает.

- Установщик создаёт ключ при установке и обновлении и никогда не пересоздаёт.
- Панель без сохранённых секретов стартует и без ключа; как только
  зашифрованные строки есть, а ключа нет, она отказывается стартовать, а не
  отдаёт пустые подписки.
- Копия ключа хранится отдельно от БД ([backup и
  restore](docs/BACKUP_RESTORE.ru.md)).

```sh
# проверить, что ключ и БД из одного поколения (печатает счётчики, не значения)
docker compose exec panel python -m panel.cli master-key-verify --path /run/panel/master-key

# ротация: новый активный ключ, перешифровка батчами, проверка, сужение keyring
docker compose exec panel python -m panel.cli master-key-rotate --path /run/panel/master-key
```

Ротация идёт overlap-first: файл держит оба ключа, пока строки
перешифровываются, поэтому прерванная ротация оставляет всё читаемым. После
завершения снимите свежую копию ключа.
