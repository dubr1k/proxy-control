...
**Egress (v0.4).** Поколение может нести необязательную секцию `egress` — по протоколам
(`naive`, `mieru`) `EgressDocument {backend, policy_id, policy_revision, document,
digest}`: политика маршрутизации этого узла, скомпилированная под его backend
([docs/ROUTING.ru.md](docs/ROUTING.ru.md)). Центр кладёт её только для узла, чей
`identity.capabilities` содержит `egress.v1` (строгая модель узла v0.3 отвергла бы весь
документ), и только для политик, которые оператор применил (или откатил): черновик не
публикуется. Поле опускается в проводной форме и в digest, когда сказать нечего, — поэтому
любой документ без egress сохраняет digest, который был у него в v0.3, в обе стороны
связи разных версий. На узле секция применяется **после ресурсов**, через те же адаптеры
менеджеров, что и локальный экран, идемпотентно (менеджер, уже работающий с этим digest,
повторно не трогается), с `operation_id = <guid>:<generation>:egress:<protocol>`;
неудачная секция делает поколение `failed`, как ресурс, — пользователи при этом
провижены, — а отчёт несёт `egress: {protocol: {state: converged | failed | unsupported,
revision, digest, error}}` (`managed_egress` на узле). Центр переводит политику в
`applied` на ревизии, названной секцией, или в `failed` с кодом узла — по устоявшемуся
отчёту о поколении, которое он хочет сейчас; отсутствие секции в следующем поколении
значит «оставить egress как есть», а `unlink` никогда не трогает data plane. Пока узлом
...
1. **Heartbeat** — `GET identity` + `GET status`. Успех записывает `online`, latency,
   ...
   или статус ошибки (401/403 при плохом ключе, 404 от панели без этого API, голая
   страница 502/503/504 от reverse proxy перед остановленной панелью) записывает
   `offline` с классом и кодом сбоя (никогда — телом ответа) в `last_error`, сохраняя
   последний хороший отчёт. **Отказ, который написала сама панель** — 429 или 5xx с её
   JSON `{detail, code}` (заминка менеджера) — только записывает ошибку и откладывает
   доставку: узел жив, а не упал. Событие — `node.up` или `node.down`
...
их**. По умолчанию он **автоматический** (галочка «Подхватывать пользователей панели
автоматически» в диалоге связи, поле `auto_import`): на каждом heartbeat центр читает
inventory узла и принимает всех, кого узел ведёт сам, — как 3x-ui считает e-mail одним
человеком, так и здесь одно имя учётной записи во всех протоколах и на всех узлах — один
клиент центра с этим именем (клиент, созданный вручную с тем же именем, тоже подходит).
Пользователь, созданный на узле позже, подхватывается следующим heartbeat'ом; узел,
которым владеет другой центр, — никогда. Строка аудита `node.import` с `auto: true`
пишется от системного актора. Галочку можно снять и импортировать вручную — из диалога
подключения (после «Проверить») или позже из вкладки «Пользователи» карточки узла →
...
**Удаление на центре** («Удалить», `DELETE /api/nodes/{id}`) отклоняется с 409, пока
остаётся хоть один *выданный* (provisioned) доступ на узле, включая ещё не подтверждённые
узлом удаления (подтверждённое удаление purge-ится, значит строка `deleted` — это аккаунт,
ещё живой на узле) — сначала удалите доступы клиентов и дождитесь отчёта узла, чтобы
абоненты не потеряли их молча и ни один аккаунт не остался живым локальным пользователем. *Импортированные* доступы не
