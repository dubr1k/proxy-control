...
- **The MCP server** (`compose.mcp.yaml`, container `proxy-control-mcp`, central panel only): the panel's API as Model Context Protocol tools for Claude Code and Claude Desktop over `https://<mcp domain>/mcp` with a bearer token. Tools are built from the panel's OpenAPI schema (one per operation, `<method>_<path>`), curated ones (`overview`, `create_client`, `client_subscription`, `set_client_placement`, `versions_check`, `versions_update`, `routing_preview`, `routing_apply`, `audit_tail`) come with hand-written descriptions, irreversible operations require `confirm: true`, and every call goes through an `admin` API key named `mcp`, so the audit trail shows it as `via: api-key`. The panel serves `GET /api/openapi.json` to owners and admins; `python -m panel.cli api-key-create` / `api-key-revoke` issue and revoke keys without a session ([docs/MCP.en.md](docs/MCP.en.md)).
