...
Инструменты дают модели «руки», скиллы — порядок действий для типовых задач оператора. Они лежат в
каталоге [`skills/`](../skills/) репозитория, по одному `SKILL.md` на задачу, и ставятся на
машину, где работает Claude Code, Codex или OMP — не на сервер:

```bash
# Claude Code
cp -r skills/proxy-control-* ~/.claude/skills/
# Codex CLI и другие клиенты, читающие общий каталог
cp -r skills/proxy-control-* ~/.agents/skills/
```

| Скилл | Когда срабатывает |
|---|---|
| `proxy-control-granting-access` | выдать человеку доступ, отдать или ротировать ссылку подписки, поменять узлы и протоколы клиента |
| `proxy-control-morning-overview` | «как дела у панели и узлов», обзор перед изменениями, что было за сутки |
| `proxy-control-updating-components` | проверить и поставить обновления Telemt, mita, NaiveProxy, Xray-router, панели |
| `proxy-control-diagnosing-access` | «подписка стоит, а не работает», протокол не появился в приложении, трафик пропал |
| `proxy-control-changing-routing` | блокировки, «напрямую», выход через WARP или свой прокси: политика, предпросмотр, применение, откат |

Скиллы знают то, чего нет в описаниях инструментов: ключи вариантов подписки и какому
приложению какой вариант нужен, что кириллическое имя без `runtime_username` даст аккаунт
`client`, что доступ на связанный узел ждёт как `pending_remote`, порядок и стоп-правила при
обновлениях, какие причины предпросмотра означают «нужен роутер». Проверены на боевой панели
сценариями «без скилла / со скиллом»; правки — через pull request с таким же прогоном.
