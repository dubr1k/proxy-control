...
а в базе панели хранится только хэш токена. Не добавляйте логирование запросов на этот путь и не
вставляйте ссылку подписки в тикеты.
...
1. **Сначала обновите узлы, затем центр.** На каждом хосте в каталоге проекта:
   `docker compose up -d --build --wait panel` с сохранённым набором overlays. Миграции 9–12
...
отказывается стартовать на базе со схемой 12 («database schema 12 is newer than this
