...
**ВЫПУЩЕНО**: тег `v0.8.0-beta.1` на `a30f6e3` (`lab-sha256: efe52922…`), две сборки из чистого клона,
workflow `Release` `35384780255` — success, pre-release опубликован, тело релиза — заметка с абсолютными
ссылками (25 ссылок, все 200), блок «Архив» — коммитом `93c0cf9`. Спека: `docs/superpowers/specs/2026-09-18-v0.8-exits-and-rules-design.md`.
...
1. Замечание владельца: на ams-server блок Geodata не виден — там нет роутера (сервисы нативные);
...
2. Свежие установки стоят на списках пина с выключенным автообновлением; AMS_Z — на Loyalsoldier
   с автообновлением (включила живая проверка, оставлено по просьбе владельца).
