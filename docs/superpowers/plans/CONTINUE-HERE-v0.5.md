...
Обновлено: 2026-09-16, вечер. **Tasks 1–15 сделаны: гейт зелёный на финальном дереве, живая проверка на
AMS_Z пройдена (53/53), релизная заметка заполнена.** Это последняя стадия vNext. Тега и публикации
...
- Ветка: `feature/vnext-v0.5-xray-router` (от `feature/vnext-v0.4-routing` = `v0.4.0-beta.1`, `d5dc752`).
  `VERSION` = `0.5.0-beta.1`. Гейт пройден на дереве `6622208` (`full`: 2145 passed / 2 skipped, 485,7 с; `routing`, `router`) и
  `ef400ff` (`compose`, `lab-host`, `lab-sha256` `5de24c49d322d42a2e3972dfa9f9b12a8b0a3792c31ea69e540e7188cc9dfbb5`;
...
| `ef400ff`, `2f06770`, `6622208` | 15 | гейт: мастер ищет архив под `--root` (тесты не зависят от узла); реальный runner роутера видит свой Compose-сервис (найдено на AMS_Z); пробы лаборатории через Интернет получают вторую попытку при сетевом сбое |

## Гейт (Task 15) — итог

`full` на `6622208` (2145 passed / 2 skipped, 485,7 с), `compose`, `lab-host` (LAB_RESET=1, 17 сценариев,
