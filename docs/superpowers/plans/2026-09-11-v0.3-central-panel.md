...
TOKEN = open("/opt/mtproxy-shared443/secrets/telemt-api-token").read().strip()
...
async def test_apply_is_idempotent_and_survives_a_restart_mid_apply(world):
    ...
    assert again == observed.model_copy(update={"reported_at": observed.reported_at})
...
LINKS_V11 = Migration(11, "fleet-v2-links", (
    ...
    """CREATE TABLE IF NOT EXISTS desired_generations (
      ...
      generation INTEGER NOT NULL, digest TEXT NOT NULL, document_json TEXT NOT NULL,
...
class DesiredStore:
    ...
    @staticmethod
    def insert(db, node_id: str, document: GenerationDocument, digest: str) -> None:
        db.execute("""INSERT INTO desired_generations(node_id,generation,digest,document_json,previous_generation,created_at,created_by)
                      VALUES(?,?,?,?,?,?,?)""",
                   (node_id, document.generation, digest, document.model_dump_json(), document.previous_generation,
...
def publish(db, clients_store, desired: DesiredStore, *, node_id, master_guid, created_by="system", now=None) -> int | None:
    ...
    digest = canonical_digest(document.model_copy(update={"generation": previous, "previous_generation": max(previous - 1, 0)}))
    if latest and latest["digest"] == digest:
        return None
    desired.insert(db, node_id, document, canonical_digest(document))
    return document.generation
```

Digest сравнивается с «тем же содержимым под номером предыдущего поколения», иначе номер сам менял бы digest. Хранить в `desired_generations.digest` именно `canonical_digest(document)`; для сравнения `latest` использовать поле `content_digest` — добавить колонку `content_digest TEXT NOT NULL DEFAULT ''` в `LINKS_V11` и считать её как digest документа с `generation=0, previous_generation=0, created_at=0` (чистый контент). Это проще, чем пересчёт «под номером предыдущего»: `content_digest(document) = canonical_digest(document.model_copy(update={"generation": 1, "previous_generation": 0, "created_at": 0, "created_by": ""}))`. Использовать `content_digest` в `publish`, хранить обе.
...
- Produces: `FleetPusher(database, links, desired, secrets, clients, provisioning, events, *, interval=15.0, clock=time)` с `async tick() -> None` (один проход по всем включённым связям), `async sync_node(node_id) -> None` (heartbeat + push при `config_dirty`/`desired > acknowledged` + обработка ответа), `async run_forever()`, `stop()`; `Settings.fleet_heartbeat_seconds: int = 15`; `ProvisioningService.start` для узлов с `transport='panel'` не делает preflight, пишет грант (`observed_state="pending"`), секрет `pending`, шаг `{"status": "remote"}`, вызывает `self.clients.notify(db, client_id)` (→ `publish`), операция `pending_remote`; `run()` пропускает шаги `remote`; `ProvisioningService.remote_applied(grant_id, state: str, *, error=None)` — вызывается pusher'ом: `enabled` → секрет `active`, `observed_state="enabled"`, шаг `active`, операция `succeeded`, если все шаги `active`; `failed` → шаг `error` (операция остаётся `pending_remote`, UI показывает ошибку узла).

- [ ] **Step 1: Failing tests (pusher)** — фикстура `pair` из Task 8 (вынести в `panel/tests/fleet_v2_fixtures.py` и импортировать).
...
from panel.tests.fleet_v2_fixtures import ACTOR, pair  # noqa: F401
...
- [ ] **Step 1: Failing tests (central routes)** — на паре приложений из `fleet_v2_fixtures` с `httpx.ASGITransport` для центра тоже (сессия owner через `login`).
...
from panel.tests.conftest import login
from panel.tests.fleet_v2_fixtures import pair  # noqa: F401


@pytest.fixture
async def central_http(pair):
    node, central, plaintext = pair
