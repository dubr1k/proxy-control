...
пройдена, релизная заметка заполнена, скриншоты и smoke экрана в браузере сняты; публикация идёт по
поручению владельца («если всё ок, публикуй релиз…»).** Тег `v0.4.0-beta.1` ставится на коммит с этой
правкой; итог публикации — в разделе «Публикация» (дописывается после релиза). Владельцу остаются
...
`docs/releases/v0.4.0-beta.1.md` с абсолютными ссылками (`gh release edit --notes-file`). Итог — ниже, после релиза.
