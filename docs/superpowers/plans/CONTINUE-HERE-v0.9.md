...
Состояние на 2026-09-19 (день UTC), ветка `main`, `VERSION = 0.9.0-beta.1`. **ВЫПУЩЕНО**: тег `v0.9.0-beta.1`
на `7072cb3` (`lab-sha256: 1c3ada63…`, две сборки из чистого клона `/root/release-check-v09` на ams-test, байты совпали),
workflow Release `35444780348` success, pre-release опубликован (4 файла, архив 8 067 419 байт, опубликованная
сумма = стендовой).
...
1. Тег и Release сделаны (см. шапку); блок «Архив» в заметке добавлен коммитом после тега. Следующая версия —
   новая ветка задач от владельца.
2. Вторая волна раскатки (мобильные правки, 20575c5) прошла 12:58–12:59 UTC: PHASE_B_OK на всех
   четырёх, PHASE_C_OK на AMS_Z/AMS_P/AMS_R; на ams-server фаза C ожидаемо жалуется, что naive-/mieru-manager
   пересозданы (12:47, включение роутера) и Caddyfile/mita config изменились (подключение к роутеру) —
   остальные 19 проверок ok, все контейнеры healthy. `markMobileNavEdges` есть в образе панели на всех хостах.
3. Временный owner `v09-tmp` на ams-server удалён через API после проверок (см. журнал сессии); файл пароля
   `/root/v09/tmp-owner.pass` стёрт. Хелпер `/root/pc-call.py` оставлен (без секретов).
