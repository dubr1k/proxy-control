...
- [ ] **Step 1: rsync и полный прогон на стенде**

```bash
rsync -a --delete --no-owner --no-group --exclude graphify-out --exclude .venv /Users/dubr1k/Syncthing/development/proxy-control/ ams-test:/root/dev/proxy-control/
ssh ams-test 'cd /root/dev/proxy-control && .venv/bin/ruff check panel && setsid nohup sh -c "cd panel && ../.venv/bin/pytest -q > /root/gate-v10-unit.log 2>&1; echo UNIT_EXIT=\$? >> /root/gate-v10-unit.log" > /dev/null 2>&1 < /dev/null &'
```

Опрос: `ssh ams-test 'tail -5 /root/gate-v10-unit.log'` до `UNIT_EXIT=0`. Ожидание: всё зелёное (в v0.9 было 2342 passed / 2 skipped; теперь на ~15 больше).

- [ ] **Step 2: Гейт full так же, как для v0.9**

Взять `/root/gate-v09.sh` на стенде (или `gate-v07.sh`, если v09 нет), скопировать в `/root/gate-v10.sh`, заменить имена логов на `gate-v10*`, запустить отсоединённо; ждать `GATE_V10_EXIT=0`. Тир `ui` обязателен: он прогоняет headless-браузер по реальной панели.
