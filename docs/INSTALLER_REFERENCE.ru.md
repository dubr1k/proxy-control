...
`[ingress]` необязателен. Он применяется только если уже работающий чужой
`stream` отправляет PROXY protocol всем backend'ам, а штатные private listeners
Telemt, панели или Caddy не принимают этот заголовок. До плана оператор поднимает
свой loopback-only bridge, который принимает PROXY protocol на указанном
`127.0.0.1:port`, выбирает backend по TLS SNI и передаёт **исходный TLS без
PROXY header** на `8445` (Telemt), `panel_tls_port` (панель/подписки; по
умолчанию 8443) либо `4443` (Naive). Если чужой Nginx уже занял 8443 с
`proxy_protocol`, задайте другой свободный loopback-порт, например 10446:
установщик создаст свой panel TLS vhost именно там. `panel_tls_port` и порт bridge
обязаны различаться. `skip_renewal_dry_run = true` — явное временное исключение только для уже
существующего lineage с локально проверенными trust, exact SAN, ключом и сроком действия: не
запускается сетевой `certbot renew --dry-run`; выпуск/renewal новых или невалидных сертификатов
оно никогда не пропускает. Установщик проверяет только существование TCP listener bridge и направляет в него только
свои SNI; он не меняет чужие SNI, не создаёт bridge и не ослабляет проверку DNS.
Отсутствующий listener — жёсткая остановка плана. Это сохраняет PROXY protocol и
реальные IP для существующих backend'ов; новые private backend'ы получают
loopback-адрес bridge. Bridge не принадлежит установщику: сохраните его
конфигурацию отдельно и проверьте реальные TLS/SNI-маршруты до и после установки.
Этот opt-in доступен только в локальном патче, не в опубликованном теге
v0.13.0-beta.1. На домашнем стенде пока не пройдена приёмка Naive (curl HTTP/2
CONNECT завершается кодом 35); до исправления и проверки настоящим клиентом
не считать интеграцию готовой и не обходить проверку установщика.
...
без буферизации, таймаут чтения 3600 с; `/healthz` и всё остальное отвечают 404), и включает
адаптер `mcp`. Без ключа MCP-сервер не устанавливается. `PANEL_ALLOWED_HOSTS` не меняется:
контейнер MCP ходит к панели с её собственным `Host`.
...
- `credentials/handoff.json`, режим `0600` внутри каталога `0700`, содержит
  учётные данные, созданные установкой: при заданном `domains.mcp` — адрес
  `https://<mcp>/mcp` (`mcp_url`) и bearer-токен MCP (`mcp_token`). Публичный отчёт
  на него не ссылается, не цитирует и не пересказывает — он лишь отмечает, что MCP включён.
