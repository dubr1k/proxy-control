...
| Гейт | Результат |
| --- | --- |
| `scripts/dev/remote-gate.sh full` (с `VERIFICATION_STRICT=1`) | `REMOTE_GATE_FULL_OK` 2026-09-18 на финальном дереве `0371c69` (pytest **2291 passed / 2 skipped**, 598,5 с; матрица без `gap`): образы panel/mieru-manager/xray-router, ruff, unittest `test_deploy` (34), `route-coverage.py` (0 проблем), doc-links, JS-синтаксис, `bash -n`/shellcheck, `systemd-analyze verify`, `git diff --check`. Раньше — на `bbcab08` (2283 passed) и `8fbf13d` (2285 passed) |
...
| `LAB_RESET=1 LAB_KEEP_INSTALL=1 scripts/dev/remote-gate.sh lab-host` | `REMOTE_GATE_LAB_HOST_OK` на `0371c69`: 18 `passed` — install 121,9 с, repair 43,2 с, backup-restore 62,4 с, reboot-recovery 66,1 с, fleet 196,8 с; uninstall/coexistence `skipped` (узел оставлен tier'ам ниже) |
...
| `lab-sha256` архива гейта | `ba5e969056751abf68f4363e0d9b18782a8c986ccd429e3fef97331507328a1d` — дерево `0371c69`, все tier'ы выше (install `lab-host` и настоящая установка ams-test — из этого архива); раньше `a2dc9379…` (`bbcab08`) и `920be8f8…` (`41ec41e`). Digest тегированного дерева — в аннотации тега и в блоке «Архив» |
