...
- **Документация.** `FLEET.*.md` переписаны под v2 (v1 — раздел «Legacy transport v1»), `PANEL.*.md`, `docs/OPERATIONS.*.md` §11, `docs/UPGRADING*.md` (миграции 9–12, порядок «узлы → центр», rsync `VERSION`), `docs/COMPATIBILITY.md` (замороженный контракт панель↔панель), `docs/VNEXT_ARCHITECTURE.md`, ADR 008.

### Обновление

Обычное обновление панели; миграции 9–12 аддитивны; сначала узлы, затем центр; хосты, обновляемые rsync, получают `VERSION` вместе с кодом. Откат — полная предыдущая генерация вместе с базой (образ v0.2 не стартует на схеме 12). Подробно: `docs/UPGRADING.ru.md`.
...
- **Documentation.** `FLEET.*.md` rewritten for v2 (v1 as «Legacy transport v1»), `PANEL.*.md`, `docs/OPERATIONS.*.md` §11, `docs/UPGRADING*.md` (migrations 9–12, order "nodes → central", rsync `VERSION`), `docs/COMPATIBILITY.md` (the frozen panel-to-panel contract), `docs/VNEXT_ARCHITECTURE.md`, ADR 008.

### Upgrading

The ordinary panel upgrade; migrations 9–12 are additive; nodes first, then the central; hosts updated by rsync receive `VERSION` with the code. Rollback is the complete previous generation, database included (a v0.2 image does not start on schema 12). Details: `docs/UPGRADING.md`.
