...
The release matrix (`qemu_lab.release_scenarios()`) covers a full install onto the coexistence topology the fixture builds - an existing shared-443 stream router and a foreign 3x-ui the installer adopts without touching. A `fresh` host and `managed-new` 3x-ui are deliberately not covered here: the fixture always installs a stream router, and `fresh` is refused against one. That matrix covers coexistence with an existing 3x-ui and an ambiguous multi-map Nginx, real protocol clients for Telemt, Naive, Mieru, VLESS TCP/XHTTP and Hysteria2, Docker build verification, repair, repeated install idempotence, restart recovery, a crash injected into every durable phase, secret scans, DNS/TLS preflight, uninstall twice, a foreign holder of a fixed identity, interrupted install/uninstall recovery, and final coexistence.
