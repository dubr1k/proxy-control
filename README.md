...
| **3x-ui** | VLESS Reality (TCP и XHTTP) и Hysteria2. Установщик умеет поставить 3x-ui с нуля или принять уже установленный, не трогая его файлы. |
...
- `managed-new` — поставить 3x-ui `3.7.0` с нуля и выписать ему сертификаты.
...
Отсюда приходят VLESS Reality (TCP и XHTTP) и Hysteria2. Установщик либо ставит
3x-ui `3.7.0` с нуля (`managed-new`), либо принимает уже установленный
(`existing`) — во втором случае он только добавляет маршруты для его доменов, а
сами файлы, база и unit 3x-ui остаются байт в байт прежними; стенд это
проверяет хешем до и после прогона.
