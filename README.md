...
| **Панель** | Роли владельца, администратора и наблюдателя; API-ключи с областями `admin \| monitor \| node-sync` (v0.3). Аудит без секретов в транзакции изменения, разовая выдача доступов, управление квотами, версионные миграции базы. |
...
- **Подписка.** Каждому клиенту — одна ссылка
  ...
  `clash` (mihomo, TCP и UDP), `manifest`, `html`. Токен хранится хэшем и
...
и UDP, canary-скан логов и дампа базы не находит ни токенов, ни паролей — и
...
и живая проверка «центр → узел-выход». Не заявляются как завершённые: мост в 3x-ui,
canary-раскатка политик, UDP через роутер, история метрик и обновление панели узла из центра,
а также бухгалтерская точность учёта трафика.
