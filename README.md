...
мастер. Он спрашивает про профиль, домены, почту для сертификатов и режим
3x-ui, а на выходе пишет файл конфигурации — обычный TOML, который вы можете
...
Любой профиль может дополнительно работать с 3x-ui — поле `three_xui.mode`:

- `none` — 3x-ui не трогаем совсем;
- `existing` — принять уже установленный: установщик добавит маршруты для его
  доменов и не изменит ни одного его файла.
...
- `managed-new` — поставить 3x-ui `3.7.0` самому: установщик выпишет ему
  ...
  VLESS Reality TCP, VLESS Reality XHTTP и Hysteria2. Требует чистого сервера:
...
Начальный пароль владельца лежит в
`/opt/mtproxy-shared443/secrets/panel-bootstrap-password` с режимом `0600`.
Прочитайте его через защищённую консоль, сразу войдите на
...
Отсюда приходят VLESS Reality (TCP и XHTTP) и Hysteria2. Установщик принимает
уже установленный 3x-ui (`existing`): он только добавляет маршруты для его
доменов, а сами файлы, база и unit 3x-ui остаются байт в байт прежними — стенд
проверяет это хешем до и после прогона. Ставить 3x-ui с нуля он пока не умеет,
см. `three_xui.mode` выше.
...
точность учёта трафика, установка 3x-ui с нуля (`three_xui.mode =
"managed-new"` — установщик отвергает этот режим при построении плана) и egress
через WARP, у которого нет автоматической приёмки: включив `warp = true`,
проверьте туннель сами.
