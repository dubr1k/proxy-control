...
Третий режим, `managed-new` (поставить 3x-ui с нуля), в этом релизе **не
поддерживается**: он ставит и запускает панель, но не создаёт ни одного
инбаунда, ни ключей Reality, ни учётных данных, то есть оставляет пустую
панель там, где конфигурация назвала VLESS и Hysteria2. Установщик отвергает
этот режим при построении плана, а не в середине установки. Поставьте 3x-ui
сами и укажите `existing`.
