...
|---|---|
| **Xray / 3x-ui** | **Только выбранный трафик.** Правила привязаны к `warp_domains`; всё остальное уходит напрямую, а обязательное финальное правило оставляет несовпавший трафик прямым. На практике это недоступно: единственный поддерживаемый режим — `existing`, а принятым экземпляром установщик не управляет и его маршрутизацию не трогает. |
