...
|---|---|
| **Xray / 3x-ui** | **Только выбранный трафик.** Правила привязаны к `warp_domains`; всё остальное уходит напрямую, а обязательное финальное правило оставляет несовпавший трафик прямым. Доступно только в режиме `managed-new`: принятым экземпляром (`existing`) установщик не управляет и его маршрутизацию не трогает. |
