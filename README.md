...
Mieru использует внешний процесс `mita` версии **3.35.x или 3.36.x**. Он не входит в MIT-репозиторий и образы; бинарный файл устанавливается оператором отдельно.
...
Для другой поддерживаемой версии или архитектуры используйте URL и оба digest из одной pinned строки. Проверяйте именно digest исполняемого файла, а не только пакета.
...
Если мобильный путь теряет крупные сегменты обратного потока, `deploy/mieru-mss-clamp.service` может закрепить измеренный TCP MSS только для listener Mieru, не меняя посторонние firewall rules. Это только opt-in: устанавливайте unit после подтверждения сигнатуры `Send-Q`/retransmission/RTO из [MIERU.ru.md](MIERU.ru.md).
...
Полный цикл релиза — установка, повторная установка, `repair`, восстановление после перезагрузки, прерванная фаза, отчёт, удаление и сосуществование на общем 443 — прогоняется на настоящем архиве релиза в двух стендах: одноразовый systemd-контейнер и одноразовый сервер на голом железе. Приёмка каждого протокола выполняется реальными клиентами: обмен `resPQ` для MTProto, cover HTTPS и аутентифицированный `CONNECT` с учётом закрытого туннеля для NaiveProxy, официальный клиент Mieru на каждом транспорте.

Прогон под QEMU (`make lab-full`), регистрация Fleet в боевой среде и бухгалтерская точность учёта трафика по-прежнему не заявляются как завершённые этапы выпуска.
