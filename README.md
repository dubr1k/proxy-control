...
WARP — одна loopback-точка **SOCKS5** на `127.0.0.1:45000` по умолчанию.
...
Параметры находятся в `[three_xui]`; `warp_port` по умолчанию равен `45000`
...
| **NaiveProxy** | При пустом `warp_domains` — весь туннелируемый трафик через `upstream socks5://127.0.0.1:45000`. При заданных селекторах остаётся direct. |
