...
| **3x-ui** | VLESS Reality (TCP и XHTTP) и Hysteria2. Установщик принимает уже установленный 3x-ui и делит с ним порт 443, не трогая ни одного его файла. Ставить 3x-ui с нуля он пока не умеет. |
...
локальную точку SOCKS5 `127.0.0.1:45000`. Если WARP-клиента там нет, отвечайте
«нет» — иначе туннели упрутся в несуществующий upstream.
...
4. проверяет каждый по открытому порту. Это важно: 3x-ui сохраняет инбаунд,
   который Xray не станет обслуживать, и продолжает показывать его включённым,
   ничего не написав в журнал. Открытый порт — единственное доказательство.
...
> [!NOTE]
> Подписка 3x-ui наружу пока **не публикуется**. Она живёт на своём порту и
> после установки доступна только с самого сервера. Причина в том, как делится
> порт 443: Nginx выбирает получателя по имени в TLS-приветствии, один домен —
> один адресат, а панель и подписка слушают разные порты, поэтому подписке
> нужен собственный домен. Он появится вместе с автоматической публикацией
> подписки; сейчас её нет, и обещать её мы не станем.
...
Флаг живёт в секции `[three_xui]`, но относится не к 3x-ui: его читают
NaiveProxy и Mieru, поэтому мастер спрашивает о нём в профилях с этими
протоколами. Если WARP-клиента по этому адресу нет, не включайте флаг — иначе
туннели пользователей упрутся в несуществующий upstream.

Три протокола обращаются с этой точкой **по-разному**, и это сделано намеренно:

| Протокол | Что уходит через WARP |
|---|---|
| **Xray / 3x-ui** | **Ничего.** Принятым экземпляром (`existing`) установщик не управляет и его маршрутизацию не трогает, а доменная маршрутизация через `warp_domains` существует только для управляемого 3x-ui, которого в этом релизе нет. |
...
То есть сессия пользователя Naive или Mieru целиком уходит через WARP. При
`warp = false` нигде не появляется ни upstream, ни правило WARP, а
egress-правило Mieru остаётся `DIRECT`.
