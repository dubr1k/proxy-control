...
- **[Руководство оператора v0.6](docs/releases/v0.6-operator-guide.ru.md)** — один документ на всё:
  ...
  маршрутизация egress и Xray-router с примерами политик, обновления и резервные копии, сквозные
  чек-листы.
- **[AGENTS.md → «Эксплуатационный протокол для ИИ-агентов (v0.6)»](AGENTS.md)** — те же операции как
...
**Бета-выпуски (v0.2.0-beta.1 … v0.6.0-beta.1).** `install-bootstrap` отклоняет
...
распакованного архива), и именно так поставлены v0.2, v0.3, v0.4, v0.5 и v0.6:
...
scripts/install-release.sh --version 0.6.0-beta.1 --sha256 <lab-sha256 из заметки о выпуске>
```
...
настоящего 3x-ui, учение backup/restore — и исправления найденного (см. заметку о выпуске). Не заявляются как завершённые: мост в 3x-ui,
canary-раскатка политик, per-grant и UDP через роутер, транзитивные узлы, история метрик и
обновление панели узла из центра, а также бухгалтерская точность учёта трафика.
