...
С v0.3 панель подключает другие панели из web-интерфейса: на панели-узле владелец
создаёт API-ключ `node-sync`, на центре «Узлы → + Панель» принимает URL и ключ, затем
импорт существующих пользователей. Дальше центр держит связь сам (heartbeat),
выдаёт, ротирует и отзывает доступы на узле и обновляет его компоненты; на хостах
не появляется ничего, кроме образа панели. Полное описание: [FLEET.ru.md](FLEET.ru.md).
